%This program will model number density vs. radius for the photosphere of an AGB star.
...
for i = 1:Maxsteps
  ...
  nu = [alpha*((pi*m)/(8*k*[4000 - (25000/3)*(((r-r4000K)*r4000K)/power(r,2))]))^(0.5)]*(1/(100^3));
  xplot(i) = r;                %Record for plotting.
  yplot(i) = nu;
  
end

%Plotting number density as a function of radius.
plot(xplot,yplot);
grid on;
xlabel('Radius of the Star(meters)');
ylabel('Number Density (atoms/cc)');
title('Number Density vs. Radius');

print("Model_Photosphere.png");
refresh;